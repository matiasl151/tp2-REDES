...
if __name__ == '__main__':
  if len(sys.argv) != 3: #controlo que entren los 3 argumentos
    print('Usage: python file_sender.py [dummy|ss|gbn] [file_name]')
    sys.exit(1)

  transport_layer = None
  transport_layer_name = sys.argv[1] #setea la capa de transporte 
  ...
  try:
    ...
    with open(file_name, 'rb') as f:
      while True:
        ...
        while not transport_layer.send(msg): #Llamo a la funcion enviar de la capa de transporte seleccionada (gbn.py)
          pass
  finally:
    if transport_layer:
