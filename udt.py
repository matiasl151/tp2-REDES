...
class NetworkLayer:
  def __init__(self, local_port, remote_port, transport_layer):
    # Puertos para los paquetes enviados y recibidos.
    self.local_port = local_port
    self.remote_port = remote_port
    # Escucha del localhost para recivir paquetes.
    self.s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    self.s.bind(('localhost', local_port))
    # self.s.setblocking(False)
    self.s.settimeout(0.5)  # En segundos.
  ...
  #Función para leer paquetes
  def _packet_reader(self):
    while not self.stop_accept_pkt:
      # Si se recibe un mensaje, notifica a la capa de transporte
      # en lugar de bloquear la lectura.
      has_msg = False
      with self.buffer_lock:
        if len(self.msg_buffer) > 0:
          has_msg = True
  ...
  # Devuelve un nuevo mensaje con errores de bit random
